...
cd "$project_dir"
state_dir="$project_dir/.cache/rocm-benchmark-v2-schedule"
mkdir -p "$state_dir/logs"
exec 9>"$state_dir/lock"
flock 9
clock="$(date +%H%M)"
log() { printf '%s %s\n' "$(date --iso-8601=seconds)" "$*" | tee -a "$state_dir/events.log"; }
name_for() { printf 'qwen35-rocm-benchmark-v2-%s' "${1//_/-}"; }
container_state() { docker inspect -f '{{.State.Status}}' "$1" 2>/dev/null || true; }
result_for() { printf '%s/benchmarks/rocm/best_epoch_v2/%s_results.json' "$project_dir" "$1"; }
complete_result() {
  local result="$1"
...
next_variant() {
  ...
  for variant in finetuned_safetensors; do
...
case "${1:-}" in
  start)
    ...
    if [[ -z "$variant" ]]; then
      log 'The v2 safetensors generation is complete; nothing to start.'
      exit 0
    fi
    name="$(name_for "$variant")"
    ...
    docker compose -f compose.train.rocm.yaml -f compose.train.rocm.patched.yaml run -d --no-deps \
      ...
      --variant "$variant" --config config/benchmark.rocm.best_epoch_v2.yaml \
