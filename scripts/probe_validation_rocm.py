...
from docker_k8s_finetune.benchmark.pipeline import (
    _backend_generate_batch,
    _generate_with_truncation_retry,
    _generation_payload,
)
...
def select_validation_records_quantiles(records: list[dict], per_category: int = 10) -> list[dict]:
    """Preselect evenly spaced reference-length quantiles without model outputs."""
    if per_category < 2:
        raise ValueError("quantile selection requires at least two records per category")
    groups: dict[str, list[dict]] = defaultdict(list)
    hashes: set[str] = set()
    for record in records:
        meta = record.get("meta", {})
    ...
    for category in sorted(groups):
        ...
        if len(ordered) < per_category:
            raise ValueError(f"Too few validation records for {category}")
        indexes = [round(index * (len(ordered) - 1) / (per_category - 1)) for index in range(per_category)]
        if len(set(indexes)) != per_category:
            raise ValueError(f"Quantile selection produced duplicate indexes for {category}")
        selected.extend(ordered[index] for index in indexes)
    return selected
...
def _load_inputs(root: Path, config: dict) -> tuple[list[dict], dict]:
    ...
    if selection == "reference_length_median_and_p90_by_category_v1":
        selected = select_validation_records(records, int(config["sample_per_category"]))
    elif selection == "reference_length_even_quantiles_by_category_v2":
        selected = select_validation_records_quantiles(records, int(config["sample_per_category"]))
    else:
        raise ValueError("Unsupported validation probe selection")
    held_out_hashes = {
...
def run(root: Path, config_path: Path, variant: str, policy: str, *, limit: int | None = None, plan: bool = False) -> dict:
    ...
    generation_mode = config.get("generation_mode", "individual_v1")
    if generation_mode == "deterministic_prompt_length_batches_v1":
        response_instruction = generation["response_instruction"]
        selected.sort(key=lambda record: (
            sum(len(message.get("content", "")) for message in prompt_messages(record, response_instruction)),
            record["meta"]["content_hash"],
        ))
    elif generation_mode != "individual_v1":
        raise ValueError("Unsupported validation probe generation mode")
    identity = {
    ...
    if "generation_mode" in config:
        identity["generation_mode"] = generation_mode
    if plan:
    ...
    def append_result(record: dict, result: dict, index: int) -> None:
        payload["records"].append({
            ...
            "generation_batch_size": result["generation_batch_size"],
            "generated_completion_tokens_total": result["generated_completion_tokens_total"],
    ...
    start = len(payload["records"])
    if generation_mode == "deterministic_prompt_length_batches_v1":
        batch_size = int(generation["batch_size"])
        if start % batch_size and start != len(selected):
            raise ValueError("Existing batched probe ends inside a deterministic batch")
        for offset in range(start, len(selected), batch_size):
            ...
            primary = _backend_generate_batch(backend, messages_batch, generation)
            chunk_results = [
                _generate_with_truncation_retry(
                    backend,
                    messages,
                ...
                for messages, generated in zip(messages_batch, primary)
            ]
