...
state_dir="$project_dir/.cache/rocm-training-v2-schedule"
mkdir -p "$state_dir/logs"
exec 9>"$state_dir/lock"
flock 9

container=qwen35-rocm-best-epoch-v2
checkpoint_dir="$project_dir/artifacts/rocm/best_epoch_v2/checkpoints/primary"
result_file="$project_dir/artifacts/rocm/best_epoch_v2/metrics/training_result.json"
...
case "${1:-}" in
  ...
  start)
    ...
    docker compose -f compose.train.rocm.yaml -f compose.train.rocm.patched.yaml run -d --no-deps \
      ...
      --config config/training.rocm.best_epoch_v2.yaml --no-export "${resume_args[@]}" >/dev/null
    touch "$started_marker"
    log 'Started ROCm best-epoch v2 training; exports remain deferred until validation passes.'
    ;;
  status)
