...
state_dir="$project_dir/.cache/rocm-training-lr1e4-v3-schedule"
mkdir -p "$state_dir/logs"
exec 9>"$state_dir/lock"
flock 9

container=qwen35-rocm-lr1e4-v3
checkpoint_dir="$project_dir/artifacts/rocm/lr1e4_v3/checkpoints/primary"
result_file="$project_dir/artifacts/rocm/lr1e4_v3/metrics/training_result.json"
...
case "${1:-}" in
  stop)
    ...
    if [[ "$(container_state)" == running ]]; then
      log 'Stopping LR 1e-4 training container at start of quiet hours.'
  ...
  start)
    ...
    checkpoint="$(find "$checkpoint_dir" -maxdepth 1 -type d -name 'checkpoint-*' -print -quit 2>/dev/null || true)"
    if [[ -z "$checkpoint" && -f "$started_marker" ]]; then
      log 'Training started previously but no compatible checkpoint exists; refusing to restart from step zero.'
      exit 1
    fi
    ...
    docker compose -f compose.train.rocm.yaml -f compose.train.rocm.patched.yaml run -d --no-deps \
      ...
      --config config/training.rocm.lr1e4_v3.yaml --no-export "${resume_args[@]}" >/dev/null
    touch "$started_marker"
    log 'Started controlled LR 1e-4 training; exports remain deferred until validation passes.'
    ;;
  status)
